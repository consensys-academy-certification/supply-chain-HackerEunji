...
contract SupplyChain {
  ...
  event mylog(uint itemId, State state);
  ...
  modifier checkCaller(address _buyorsell) {
    require(msg.sender == _buyorsell);
    _;
  }
  ...
  function addItem(string calldata _name, uint _price) checkValue(1 finney) payable external {
    ...
    emit mylog(itemId, items[itemId].state);
  }
